...
function App() {
  ...
  const pathname = usePathname();
  const isNotFound = !pathname || !['/', '/workers', '/categories', '/portfolio', '/emergency', '/smartcity', '/about', '/contact', '/favorites', '/login', '/signup', '/dashboard', '/admin'].some(p => pathname === p || pathname.startsWith('/worker/'));
  
  // Helper to check if route matches worker detail pattern
  const isWorkerDetail = pathname && pathname.startsWith('/worker/');
  
  if (isNotFound && !isWorkerDetail) {
    return (
      <ErrorBoundary>
        <ThemeProvider>
    ...
    <ErrorBoundary>
      <ThemeProvider>
        <AppCtx.Provider value={value}>
          ...
          <Navbar />
          {pathname === '/' && <HomePage />}
          {pathname === '/workers' && <WorkersDirectory />}
          {pathname === '/categories' && <CategoriesPage />}
          {pathname === '/portfolio' && <PortfolioPage />}
          {pathname === '/emergency' && <EmergencyPage />}
          {pathname === '/smartcity' && <SmartCityPage />}
          {pathname === '/about' && <AboutPage />}
          {pathname === '/contact' && <ContactPage />}
          {pathname === '/favorites' && <FavoritesPage />}
          {pathname === '/login' && <LoginPage />}
          {pathname === '/signup' && <SignupPage />}
          {pathname === '/dashboard' && <ProtectedRoute><UserDashboard /></ProtectedRoute>}
          {pathname === '/admin' && <ProtectedRoute requiredRole="admin"><AdminDashboard /></ProtectedRoute>}
          {isWorkerDetail && <WorkerDetail />}
          <ThemeToggleBtn />
          <HooriBot />
          <EmergencyFloat />
          <Toast toasts={toasts} />
