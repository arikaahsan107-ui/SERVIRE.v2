...
/* ═══════════════════════════════════════════════════
   DATA (deterministic to prevent hydration errors)
═══════════════════════════════════════════════════ */
...
// DETERMINISTIC WORKERS GENERATION - NO RANDOM VALUES
function genWorkers(){
  return Array.from({length:105},(_,i)=>{
    ...
    const revs = Array.from({length: (i % 4) + 1}, (_,ri) => ({
      id: ri,
      userName: isFem ? M_NAMES[ri % M_NAMES.length] : F_NAMES[ri % F_NAMES.length],
      rating: 4 + (i % 2),
      text: REV_POOL[(i + ri) % REV_POOL.length],
      date: new Date(Date.now() - (i * 864e5)).toLocaleDateString('en-PK', {day:'numeric', month:'short', year:'numeric'}),
      verified: (i % 5) !== 0
    }));
    
    const skills = [
      {name:'Communication', pct: 70 + (i % 25)},
      {name:'Technical Skill', pct: 75 + (i % 22)},
      {name:'Punctuality', pct: 80 + (i % 18)},
      {name:'Quality of Work', pct: 72 + (i % 25)}
    ];
    
    return {
      id:i+1,
      name,
      email:`${name.toLowerCase().replace(/\s/g,'.')}@servire.pk`,
      phone:`+92 3${Math.floor(i/10) % 4}0 ${1000000 + i * 10000}`,
      avatar,
      coverImage:prof.cover,
      portfolio:prof.portfolio.map((img,pi)=>({id:pi,image:img})),
      skill,
      subSkills:[SKILLS[(i+1)%SKILLS.length], SKILLS[(i+2)%SKILLS.length]],
      experience: (i % 14) + 1,
      price: 500 + i * 45,
      rating: 4.0 + ((i % 10) / 10),
      totalReviews: ((i % 4) + 1) * 18 + 12,
      reviews: revs,
      city: CITIES[i % CITIES.length],
      zone: ZONES[i % ZONES.length],
      availability: (i % 3) !== 0 ? 'available' : 'busy',
      verified: (i % 5) !== 0,
      completedJobs: (i % 500) + 20,
      joinDate: new Date(Date.now() - (i * 30 * 864e5)).toLocaleDateString('en-PK',{month:'long',year:'numeric'}),
      emergencyAvailable: (i % 2) === 0,
      languages: ['Urdu', 'English', ...((i % 3) === 0 ? ['Punjabi'] : [])],
      skillBars: skills,
      bio: `🌊 Seasoned ${skill} with ${(i % 14)+1}+ years of professional excellence.\n\n✨ Known for meticulous work, honest pricing, and complete customer satisfaction.\n\n🌿 Serving ${CITIES[i % CITIES.length]} and surrounding areas with verified credentials.`
    };
  });
}
const WORKERS = genWorkers();

// DETERMINISTIC EMPLOYEES GENERATION
const EMPLOYEES = Array.from({length:105},(_,i)=>{
  const isFem=i%2===0;
  return {
    id:i+1,
    name:isFem?F_NAMES[i%F_NAMES.length]:M_NAMES[i%M_NAMES.length],
    avatar:isFem?FALLBACK_AVATAR_F(i):FALLBACK_AVATAR_M(i),
    department:['Management','Operations','Support','Verification','Finance','HR'][i%6],
    role:['Senior Executive','Manager','Specialist','Coordinator','Analyst'][i%5],
    rating: 4.0 + ((i % 20) / 10),
    status:(i%7)!==0?'active':'on-leave'
  };
});
          ...
          {msgs.map((m,i)=>(
            <div key={i} className={`h-msg ${m.role}`}>
              ...
              <div className="h-msg-bubble">
                ...
                {m.nav&&<button className="h-nav-btn" onClick={()=>{router.push(m.nav.to);setOpen(false);addToast('✨ Navigating...');}}>🔗 {m.nav.label}</button>}
              </div>
            </div>
          ))}
...
const Navbar=()=>{
  ...
  return (<>
    <nav className={`navbar ${scrolled?'scrolled':''}`}>
      <Link className="nav-brand" href="/" onClick={()=>setMenuOpen(false)}>✦ SERVIRE</Link>

      <div className="nav-links">
        {NAV_ITEMS.map(({to,label})=>(
          <Link key={to} className={`nav-link ${pathname===to ? 'active' : ''}`} href={to}>
            {label}
          </Link>
        ))}
        {user?.role==='admin'&&<Link className={`nav-link ${pathname==='/admin' ? 'active' : ''}`} href="/admin">Admin</Link>}
      </div>

      <div className="nav-actions">
        <Link className="btn btn-icon" style={{position:'relative',textDecoration:'none'}} href="/favorites" title="Favorites">
          💖
        ...
        {user?(
          <Link className="btn btn-ghost btn-sm" href="/dashboard" style={{textDecoration:'none'}}>👤 {user.name.split(' ')[0]}</Link>
        ):(<>
          <Link className="btn btn-ghost btn-sm" href="/login" style={{textDecoration:'none'}}>Login</Link>
          <Link className="btn btn-jade btn-sm" href="/signup" style={{textDecoration:'none'}}>Sign Up ✦</Link>
        </>)}
    ...
    {menuOpen&&(
      <div className="mobile-menu" role="navigation" aria-label="Mobile navigation">
        {NAV_ITEMS.map(({to,label,icon},i)=>(
          <Link
            key={to}
            className={`mobile-menu-item ${pathname===to ? 'active' : ''}`}
            href={to}
        ...
        {user?.role==='admin'&&(
          <>
            ...
            <Link className={`mobile-menu-item ${pathname==='/admin' ? 'active' : ''}`} href="/admin" onClick={()=>setMenuOpen(false)}>
        ...
        <div className="mobile-menu-actions">
          {user?(
            <Link className="btn btn-jade w-full" href="/dashboard" style={{textDecoration:'none',justifyContent:'center'}} onClick={()=>setMenuOpen(false)}>
              👤 My Dashboard
            </Link>
          ):(<>
            <Link className="btn btn-ghost" href="/login" style={{textDecoration:'none',flex:1,justifyContent:'center'}} onClick={()=>setMenuOpen(false)}>Login</Link>
            <Link className="btn btn-jade" href="/signup" style={{textDecoration:'none',flex:2,justifyContent:'center'}} onClick={()=>setMenuOpen(false)}>Sign Up ✦</Link>
          </>)}
        </div>
      </div>
    )}
        ...
        <div className="footer-top">
          ...
          <div className="footer-col">
            ...
            {QUICK_LINKS.map(({to,label})=>(
              <Link key={to} className="footer-link" href={to}>
...
const WorkerCard=({worker:w,onClick})=>{
  ...
  const handleClick=()=>onClick?onClick():router.push(`/worker/${w.id}`);
  const heart=e=>{
    e.stopPropagation();
    ...
    <PageWrapper>
      <div className="hero">
        ...
        <div className="hero-content">
          ...
          <div className="hero-btns">
            <button className="btn btn-jade btn-lg" onClick={()=>router.push('/signup')}>✦ Get Started Free</button>
            <button className="btn btn-outline btn-lg" onClick={()=>router.push('/workers')}>Browse Workers 🌊</button>
            <button className="btn btn-ghost btn-lg" onClick={()=>router.push('/emergency')} style={{borderColor:'rgba(255,126,182,0.4)',color:'var(--rose)'}}>🚨 Emergency</button>
          </div>
      ...
      <div className="section">
        <div className="profession-grid" style={{maxWidth:'100%',margin:'0 auto'}}>
          {PROF_GRID.map((item,i)=>(
            <div key={i} className={`profession-grid-item${item.spanTall?' span-tall':''}`} style={item.spanTall?{gridRow:'1/3'}:{}} onClick={()=>router.push('/categories')}>
      ...
      <div className="section" style={{paddingTop:'0rem'}}>
        ...
        <div className="srv-grid" style={{maxWidth:'100%',margin:'0 auto'}}>
          {SERVICES.map(s=>(<button key={s.label} className="srv-btn" onClick={()=>router.push('/categories')}><span className="si">{s.icon}</span>{s.label}</button>))}
        </div>
      </div>
      ...
      <div className="section" style={{paddingTop:'0rem'}}>
        ...
        <div className="text-center mt-4"><button className="btn btn-outline btn-lg" onClick={()=>router.push('/workers')}>View All 105 Workers →</button></div>
      </div>
    ...
    <div className="confirm-page">
      ...
      <div className="confirm-box card">
        ...
        <div style={{display:'flex',gap:'0.75rem',flexWrap:'wrap'}}>
          <button className="btn btn-jade" style={{flex:1,justifyContent:'center'}} onClick={()=>router.push('/')}>🏠 Home</button>
          <button className="btn btn-ghost" style={{flex:1,justifyContent:'center'}} onClick={()=>router.push('/dashboard')}>📋 My Bookings</button>
        </div>
      </div>
    </div>
  );
    ...
    <PageWrapper>
      ...
      <div className="detail-page">
        ...
        <div className="detail-wrap">
          <div className="container-safe" style={{ width: '100%', padding: '0' }}>
            <button className="back-btn mt-2" onClick={() => router.push('/workers')}>← Back to Workers</button>
          </div>
      ...
      <div className="cat-page">
        ...
        <div className="section">
          <div className="cat-hero-grid" style={{maxWidth:'100%',margin:'0 auto'}}>
            {cats.map(c=>(
              <div key={c.skill} className="cat-card" onClick={()=>router.push('/workers')}>
        ...
        <div className="section">
          ...
          <div className="port-masonry" style={{maxWidth:'100%',margin:'0 auto'}}>
            {allImages.filter(a=>filter==='All'||a.skill===filter).map((item,i)=>(
              <div key={i} className="port-masonry-item" onClick={()=>router.push('/workers')}>
                <img src={item.img} alt={item.label} onError={e=>imgFallback(e)}/>
...
const SmartCityPage=()=>{
  ...
  // DETERMINISTIC HEATMAP - NO RANDOM
  const HEATMAP = Array.from({length:7}, (_, rowIdx) => 
    Array.from({length:10}, (_, colIdx) => ({
      active: (rowIdx + colIdx) % 3 !== 0,
      intensity: ((rowIdx * colIdx) % 100) / 100
    }))
  );
...
const LoginPage=()=>{
  ...
  const login=async()=>{if(!validate())return;setLoading(true);await new Promise(r=>setTimeout(r,1200));const u={id:'u1',name:email.includes('admin')?'Admin User':email.split('@')[0],email,role:email.includes('admin')?'admin':'user'};setUser(u);addToast(`Welcome back, ${u.name}! 🌊`,'success');router.push(u.role==='admin'?'/admin':'/dashboard');setLoading(false);};
  return(
    <PageWrapper>
      <div className="auth-page">
        ...
        <div className="auth-box">
          ...
          <p style={{textAlign:'center',fontSize:'0.8rem',color:'var(--text2)'}}>No account? <span style={{color:'var(--jade)',fontWeight:700,cursor:'pointer'}} onClick={()=>router.push('/signup')}>Sign up ✦</span></p>
...
const SignupPage=()=>{
  ...
  const submit=async()=>{if(!val())return;setLoading(true);await new Promise(r=>setTimeout(r,1500));const u={id:'u'+Date.now(),name:form.name,email:form.email,role};setUser(u);setLoading(false);setDone(true);addToast(`Welcome to Servire, ${form.name}! 🌊`,'success');setTimeout(()=>router.push('/dashboard'),2500);};
  return(
    <PageWrapper>
      <div className="auth-page">
        <Particles/>
        <div className="auth-box">
          ...
          {done?(
            ...
            <>
              ...
              <p style={{textAlign:'center',fontSize:'0.78rem',color:'var(--text2)',marginTop:'1rem'}}>Have an account? <span style={{color:'var(--jade)',fontWeight:700,cursor:'pointer'}} onClick={()=>router.push('/login')}>Sign in →</span></p>
            </>
          )}
        </div>
      </div>
    ...
    <PageWrapper>
      <div className="dash-page">
        <aside className="dash-sidebar">
          ...
          <button className="dnav-item" style={{marginTop:'2rem'}} onClick={()=>{setUser(null);router.push('/');}}>🚪 Logout</button>
        </aside>
        <main className="dash-main">
          ...
          {tab==='saved'&&(saved.length>0?(<div className="g3">{saved.map(w=><WorkerCard key={w.id} worker={w}/>)}</div>):(<div className="empty-state"><div className="empty-icon">🌊</div><div className="empty-title">No saved workers yet</div><button className="btn btn-jade mt-4" onClick={()=>router.push('/workers')} style={{justifyContent:'center'}}>Browse Workers →</button></div>))}
      ...
      <div style={{paddingTop:'var(--nav-height)',minHeight:'100vh'}}>
        <div className="section">
          <div className="container-safe" style={{maxWidth:'100%'}}>
            ...
            {saved.length>0?(<div className="g3">{saved.map(w=><WorkerCard key={w.id} worker={w}/>)}</div>):(<div className="empty-state"><div className="empty-icon">🌊</div><div className="empty-title">No favorites yet</div><div className="empty-desc">Browse workers and tap 🤍 to save them here</div><button className="btn btn-jade mt-4" onClick={()=>router.push('/workers')} style={{justifyContent:'center'}}>Browse Workers →</button></div>)}
          </div>
        </div>
        <Footer/>
      </div>
  ...
  return(
    <PageWrapper>
      <div className="notfound-page">
        ...
        <div className="hero-btns"><button className="btn btn-jade" onClick={()=>router.push('/')}>🏠 Go Home</button><button className="btn btn-ghost" onClick={()=>router.back()}>← Go Back</button></div>
        <Particles/>
      </div>
    </PageWrapper>
  );
